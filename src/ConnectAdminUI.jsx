    ...
    return (
      <div className="space-y-4">
        ...
        <Card title="settings_json (current)" subtitle="Stored for business">
          <pre className="text-xs bg-slate-50 rounded-xl p-3 overflow-auto max-h-[260px]">{JSON.stringify(settingsByCode[activeBizCode], null, 2)}</pre>
        </Card>
      </div>
    );
  };

  const BizAuditsScreen = () => (
    <BizAudits code={activeBizCode} tz={settingsByCode[activeBizCode]?.timezone||"IST"}/>
  );

  // Mode switch chip
  const ModeChip = (
    <div className="flex items-center gap-2 rounded-xl border bg-white p-1">
      <button className={`px-3 py-1.5 rounded-lg text-sm ${mode==="biz"?"bg-slate-900 text-white":"hover:bg-slate-100"}`} onClick={()=>setMode("biz")}>Business Ops</button>
      <button className={`px-3 py-1.5 rounded-lg text-sm ${mode==="admin"?"bg-slate-900 text-white":"hover:bg-slate-100"}`} onClick={()=>setMode("admin")}>Admin</button>
    </div>
  );
      ...
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-600 to-violet-600 text-white flex items-center justify-center shadow-sm"><IconLogo/></div>
            <div>
              <div className="font-semibold">ConnectEasy • OPS</div>
              <div className="text-xs text-slate-500">Internal Console</div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {ModeChip}
            <Badge variant="gray">You: ops@connecteasy.in</Badge>
          </div>
        </div>
      </div>
          ...
          <div className="grid md:grid-cols-[240px_1fr] gap-6">
            <aside className="bg-white rounded-2xl border border-slate-100 shadow-sm p-3 h-fit sticky top-20">
              ...
              <div className="mt-3 p-3 rounded-xl bg-gradient-to-br from-blue-50 to-violet-50 text-sm">
                <div className="font-medium mb-1">Admin scope</div>
                <ul className="list-disc pl-5 text-slate-700 space-y-1">
                  <li>Create business & seed settings</li>
                  <li>Apply ruleset packs</li>
                  <li>Send welcome email</li>
                  <li>Manage invites & ops health</li>
                </ul>
              </div>
            </aside>
            <main className="space-y-6">
              {AdminScreen}
              <footer className="text-xs text-slate-500 pt-2">OPS Console • Static preview • {new Date().toLocaleDateString()}</footer>
            </main>
          </div>
        ) : (
          // Business Ops mode
          !activeBiz ? (
            <BusinessSwitcher businesses={businesses} active={activeBizCode} onPick={(code)=>setActiveBizCode(code)} />
          ) : (
            <div className="grid md:grid-cols-[240px_1fr] gap-6">
              <aside className="bg-white rounded-2xl border border-slate-100 shadow-sm p-3 h-fit sticky top-20">
                <nav className="space-y-1">
                ...
                <div className="mt-3 p-3 rounded-xl bg-gradient-to-br from-blue-50 to-violet-50 text-sm">
                  ...
                  <ul className="list-disc pl-5 text-slate-700 space-y-1">
                    <li>Edit rules (v2 builder)</li>
                    <li>Invites & token health</li>
                    <li>Per‑biz settings only</li>
                  </ul>
                </div>
              </aside>
              <main className="space-y-6">
                ...
                {{
                  ...
                  ops_health: <BizOpsHealth onRecheck={()=>{}}/>,
                  biz_settings: <BizSettingsScreen/>,
                  audits: <BizAuditsScreen/>,
                }[bizNav]}
                <footer className="text-xs text-slate-500 pt-2">Working in <b>{activeBiz.name}</b> • {activeBiz.code}</footer>
              </main>
            </div>
          )
        )}
...
export default function ConnectEasyOpsAdminUI(){
